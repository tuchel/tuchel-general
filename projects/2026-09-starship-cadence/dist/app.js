import {defaults,rate,forecast} from './model.js';
...
let H,scenarios=structuredClone(defaults),active='baseline',unit='annual',page=0,forecasts={};
...
function chart(id,{xmin,xmax,ymax,ticks,xticks,log=false,label=''}){
 ...
 const X=v=>pad.l+(v-xmin)/(xmax-xmin)*w;const Y=v=>pad.t+h-h*(log?(Math.log10(Math.max(.1,v))+1)/(Math.log10(ymax)+1):v/ymax);
...
function renderForecast(){
 ...
 const min=series[0].map((p,i)=>({x:p.year,y:Math.min(...series.map(s=>s[i][unit]))})),maxs=series[0].map((p,i)=>({x:p.year,y:Math.max(...series.map(s=>s[i][unit]))}));area(c,min,maxs,'#829bc4',.09);
 for(const [key,data]of Object.entries(forecasts)){const p=scenarios[key];path(c,data.map(d=>({x:d.year,y:d[unit]})),p.color,key==='baseline'?3:2,key==='bearish'?'5 4':'');const last=data.at(-1);c.svg.append(el('circle',{cx:c.X(last.year),cy:c.Y(last[unit]),r:4,fill:p.color}));}
 interactions(c,forecasts.baseline.map(d=>({x:d.year,...d})),p=>`<strong>${p.year} · ${unit==='weekly'?'average flights / week':unit==='cumulative'?'cumulative future launches':'launches / year'}</strong>${Object.entries(forecasts).map(([k,rows])=>`<div class="tip-row" style="color:${scenarios[k].color}"><span>${scenarios[k].name}</span><b>${fmt(rows.find(x=>x.year===p.year)[unit],unit==='weekly'?2:1)}</b></div>`).join('')}<small>${p.year===2026?'2026 includes 2 observed flights; cumulative excludes them.':'Model output · editable assumptions'}<br>Scenario spread has no assigned probability.</small>`);
 $('#chart-subtitle').textContent=unit==='annual'?'Annual launches · full-stack flight attempts':unit==='weekly'?'Average flights / week · annual total ÷ 52.18':'Cumulative launches · September 19, 2026–year end';
 $('#outcomes').innerHTML=Object.entries(forecasts).map(([k,rows])=>`<div class="outcome"><span class="label ${k}">${scenarios[k].name.toUpperCase()} · 2035</span><strong>${fmt(rows.at(-1).annual)}</strong><small>${fmt(rows.at(-1).weekly,1)} flights / week</small></div>`).join('');
 $('#forecast-table').innerHTML=`<table class="forecast-table"><thead><tr><th>Launches / year</th>${forecasts.baseline.map(d=>`<th>${d.year}${d.year===2026?'*':''}</th>`).join('')}</tr></thead><tbody>${Object.entries(forecasts).map(([k,rows])=>`<tr><td class="${k}">${scenarios[k].name}</td>${rows.map(d=>`<td>${fmt(d.annual)}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
 const p=scenarios[active],end=rate(2035.5,p,H);$('#constraint').innerHTML=`2035 limiting factor: <b>${end.limit.toLowerCase()}</b>.<br>${fmt(p.growth,2)}× Falcon learning · ${fmt(p.uptime)}% operating availability.`;
...
function renderEvidence(){
 let c=chart('#cadence-chart',{xmin:2010,xmax:2027,ymax:6,ticks:[0,1,2,3,4,5,6],xticks:[2010,2014,2018,2022,2026],label:'Flights / week'});
 path(c,H.weekly.map(w=>({x:w.x,y:w.count})),'#748599',.8,'',.35);path(c,H.weekly.map(w=>({x:w.x,y:w.smoothed})),'#62dccb',2);path(c,H.cadenceFit.points.filter(p=>p.x<=H.cutoff),'#edf2f7',2,'5 5');
 interactions(c,H.weekly,p=>`<strong>Week of ${dates(p.date)}</strong><div class="tip-row"><span>Actual liftoffs</span><b>${p.count}</b></div><div class="tip-row"><span>13-week average</span><b>${fmt(p.smoothed,2)}</b></div><div class="tip-row"><span>Fitted S-curve</span><b>${fmt(H.cadenceFit.ceiling/(1+Math.exp(-H.cadenceFit.k*(p.x-H.cadenceFit.midpoint))),2)}</b></div><small>Source: GCAT · Monday–Sunday UTC</small>`);
...
function bind(){
 ...
 $('#log-scale').onchange=renderForecast;$('#reset').onclick=()=>{scenarios[active]=structuredClone(defaults[active]);renderControls();renderForecast()};
 ...
 $('#export').onclick=()=>{const rows=[['scenario','year','annual_launches','future_launches','cumulative_future_launches','average_weekly_launches','growth_multiplier','delay_years','ceiling_per_year','fleet_by_2036','turnaround_days_by_2036','availability_percent','initial_potential_rate','as_of']];Object.entries(forecasts).forEach(([k,data])=>data.forEach(d=>{const p=scenarios[k];rows.push([p.name,d.year,d.annual.toFixed(3),d.future.toFixed(3),d.cumulative.toFixed(3),d.weekly.toFixed(3),p.growth,p.delay,p.ceiling,p.fleet,p.turn,p.uptime,p.start,H.asOf])}));const blob=new Blob([rows.map(r=>r.join(',')).join('\n')],{type:'text/csv'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='starship-forecast-2026-2035.csv';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)};

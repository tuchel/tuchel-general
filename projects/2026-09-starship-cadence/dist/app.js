import {forecastEndYear,defaults,rate,forecast,james,jamesMilestones,jamesRate,jamesForecast} from './model.js';
...
function renderControls(){
 ...
 if(isJames){
  ...
  $('#inputs').innerHTML=`<dl class="james-milestones">${jamesMilestones.map(p=>`<div><dt>${p.label}</dt><dd>${p.cadence}</dd></div>`).join('')}</dl><p class="james-extension"><b>2031–${forecastEndYear} · annual doubling</b><br>Dashed extension beyond your last dated target.</p><p class="fine">Annual totals integrate the ramp. Use Exit rate to see year-end cadence as launches/year.</p>`;
...
function renderForecast(){
 ...
 const c=chart('#forecast-chart',{xmin:2026,xmax:forecastEndYear+.25,ymax:max,ticks,xticks:window.innerWidth<600?[2026,2028,2030,forecastEndYear]:forecasts.baseline.map(d=>d.year),log});
 ...
 $('#outcomes').innerHTML=Object.entries(forecasts).map(([k,rows])=>`<div class="outcome"><span class="label ${k}">${labels[k].name.toUpperCase()} · ${forecastEndYear}</span><strong>${fmt(rows.at(-1).annual)}</strong><small>${fmt(rows.at(-1).weekly,1)} flights / week</small></div>`).join('');
...
function renderCadence(){
 ...
 const overlay=Object.fromEntries(selected.map(key=>[key,Array.from({length:225},(_,i)=>{const t=H.cutoff+(forecastEndYear+1-H.cutoff)*i/224;return {x:aligned?t-2023:t,y:weeklyRate(key,t),t};})]));
 ...
 const xmin=aligned?0:2010,xmax=aligned?17:enabled?forecastEndYear+1:2027;
 const c=chart('#cadence-chart',{xmin,xmax,ymax,ticks,xticks:aligned?[0,4,8,12,16]:enabled?[2010,2016,2022,2028,forecastEndYear]:[2010,2014,2018,2022,2026],log:log?'log1p':false,label:'Flights / week'});
 ...
 $('#overlay-note').textContent=enabled?`${aligned?'Years since debut year: Falcon 9 = 2010; Starship = 2023.':'Calendar years; Starship projections begin September 2026.'} Starship curves end December 31, ${forecastEndYear}. Same flights/week units. ${log?'Log(1 + rate) scale preserves zero-launch weeks. ':'Linear scale. '}Overlays show modeled operating rates, not annual averages. James is dashed after 2030.`:'Enable any Starship curve to compare it with Falcon. Overlays use the current scenario assumptions.';

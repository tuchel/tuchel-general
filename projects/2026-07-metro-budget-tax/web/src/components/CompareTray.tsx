...
export function CompareTray({ pinned, onUnpin, onClear, includeState = false }: Props) {
    ...
    <section className="compare-tray" id="compare" aria-label="Compare metros">
      <div className="section-head row">
        <div>
          <h2>Compare</h2>
          <p>
            Pinned metros, aligned on shared scales
        ...
        {pinned.map((m) => {
          ...
          return (
            <article key={m.cbsa} className="compare-card">
              ...
              <dl>
                <div>
                  <dt>{includeState ? 'Local+state tax' : 'Tax / person'}</dt>
                  <dd className="mono">{money(tax)}</dd>
                </div>
                <div>
                  <dt>{includeState ? 'Local+state spend' : 'Spend / person'}</dt>
                  <dd className="mono">{money(spend)}</dd>
                </div>
                <div>
                  <dt>Gap</dt>
                  <dd className="mono">{money(gap)}</dd>
                </div>
                <div>
                  <dt>Tax / income</dt>
                ...
                {m.fisc_style && (
                  <div>
                    <dt>FiSC-style tax (city)</dt>
                    <dd className="mono">{money(m.fisc_style.tax_per_capita)}</dd>
                  </div>
                )}
                <div>
                  <dt>IG share</dt>

...
const L3A: GoalCurve = {
  ...
  maxLiveAtPeak: 7,
  spawnPeriodAtPeak: 1.4,
  light: ["gridsat", "ghost", "mirror"],
  heavy: ["beetle", "mirror"],
  heavyFrom: 0.5,
};
...
export function sampleIntensity(
  ...
  // Soft floor so lulls still feel alive, hard ceiling for peaks
  const maxLive = Math.max(
    0,
    Math.round(curve.maxLiveAtPeak * (0.15 + 0.85 * intensity)),
  );
  // Lulls stretch spawn period; peaks compress it
  const spawnPeriod =
    curve.spawnPeriodAtPeak * (2.4 - 1.5 * intensity);
  ...
  return {
    ...
    beat,
    maxLive,
    spawnPeriod: Math.max(0.55, spawnPeriod),
    aggression,
...
export const SCRIPT_BEATS: Record<LevelId, { a: ScriptBeat[]; b: ScriptBeat[] }> = {
  1: {
    a: [
      ...
      { at: 0.08, kind: "crab", x: 560, z: 0.55 },
      { at: 0.14, kind: "drone", x: 640, z: 0.7, hop: 45 },
      { at: 0.3, kind: "turret", x: 900, z: 0.8, announce: "NIX: Turret nest — peel left!" },
      { at: 0.4, kind: "walker", x: 1150, z: 0.5, announce: "NIX: Walker on the road!" },
      { at: 0.44, kind: "hackbot", x: 1200, z: 0.4 },
      { at: 0.55, kind: "drone", x: 1500, z: 0.3, hop: 35 },
      { at: 0.7, kind: "walker", x: 1850, z: 0.55, announce: "NIX: Heavy contact — Pad 7 ahead!" },
      { at: 0.74, kind: "turret", x: 1920, z: 0.85 },
      { at: 0.78, kind: "crab", x: 2000, z: 0.35 },
      { at: 0.82, kind: "drone", x: 2100, z: 0.65, hop: 50 },
      { at: 0.86, kind: "hackbot", x: 2200, z: 0.45 },
    ],
    b: [
      { at: 0.1, kind: "drone", x: 2580, z: 0.45, hop: 40 },
